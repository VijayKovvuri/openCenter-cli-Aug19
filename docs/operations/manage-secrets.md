...
The `sops-age` Secret is automatically created in the `flux-system` namespace
during `opencenter cluster deploy`. The deploy bootstrap reads the local Age
private key (generated by `opencenter cluster init`) and applies it to the
cluster so that Flux can decrypt SOPS-encrypted secrets at reconciliation time.

If you need to manually recreate or rotate the secret (for example, after a key
rotation), run:

```bash
kubectl create secret generic sops-age \
  --from-file=age.agekey=$HOME/.config/opencenter/clusters/secrets/<org>/<cluster>/age/keys/<cluster>-key.txt \
  -n flux-system \
