...
Cluster YAML expresses service desired state through typed fields: enablement,
service settings, image settings, and other fields documented in the
[Configuration Schema](../reference/configuration-schema.md). Renderer
selection, descriptor or stage topology, generated-file ownership, and raw Helm
override values are internal implementation details, not customization points.
Do not add or edit renderer metadata such as `single_stage`, `base_only`,
`source_name`, `override_values_renderer`, or `has_override_values` in a cluster
file.

If a required operator setting is missing, use the service's typed configuration
contract and request that field be added rather than encoding renderer behavior
in YAML. For manifests or values outside that contract, use the service overlay's
user-owned `custom/` directory. Generator-owned output is regenerated by
openCenter and must not be used as a hand-authored customization surface.

For older v2 files that contain renderer metadata, follow [Normalize Legacy v2
Renderer Metadata](normalize-legacy-renderer-metadata.md) before validating or
...
Override the default image repository and tag with the nested `image` fields:
...
opencenter:
  services:
    vsphere-csi:
      enabled: true
      image:
        repository: "my-registry.example.com/csi-vsphere"
        tag: "v3.3.1"
```

## GitOps Source Configuration

For managed services, configure the nested GitOps `source` fields. Use either a
pinned `release` or a tracking `branch`:
