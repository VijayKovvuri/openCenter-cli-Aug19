...
**❗ IMPORTANT**\
Ensure ranges don’t overlap with:
...
**❗ IMPORTANT**\
`vrrp_ip` is required when `use_octavia=false` and `vrrp_enabled=true`.
...
**⚠️ WARNING**\
Changing CNI plugin or network ranges on existing clusters requires cluster rebuild.
